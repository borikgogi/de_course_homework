...
def _download_task(ds, **kwargs):
    file_path = download(ds=ds, landing_dir=LANDING_DIR)
    return file_path


def _validate_task(ti, **kwargs):
    file_path = ti.xcom_pull(task_ids="download_archive")
    validate(path=file_path)


def _load_task(ds, ti, **kwargs):
    file_path = ti.xcom_pull(task_ids="download_archive")
    rows = load_to_duckdb(path=file_path, ds=ds, db_path=DB_PATH)
    return rows


def _notify_task(ds, **kwargs):
    summary = summarize(ds=ds, db_path=DB_PATH)
    logging.info("ETL completed for %s: %s", ds, summary)
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["homework", "gharchive", "duckdb"],
) as dag:
